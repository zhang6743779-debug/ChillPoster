...
def _is_upgrade_restart_handoff_task(run_id, task):
    task_type = (task or {}).get("task_type") or _infer_task_type(run_id, (task or {}).get("name", ""))
    if task_type != "upgrade":
        return False
    detail = (task or {}).get("detail")
    step = str((detail or {}).get("step") or "") if isinstance(detail, dict) else ""
    percent = _coerce_float((task or {}).get("percent"), 0.0)
    return step in {"helper", "restarting"} or percent >= 85


def _mark_upgrade_handoff_finished_locked(run_id, task, now=None):
    now = now or time.time()
    record = dict(task or {})
    detail = record.get("detail") if isinstance(record.get("detail"), dict) else {}
    detail = dict(detail)
    detail["step"] = "done"
    detail["message"] = detail.get("message") or "升级助手已接管，服务已完成重启恢复"
    record.update({
        "name": "系统升级",
        "percent": 100,
        "status": "finished",
