...
from core.configs import APP_CACHE_DB_FILE, BACKUPS_DIR, MEDIA_LIBRARY_CACHE_DB_FILE  # noqa: E402


APP_CACHE_TABLES = (
    "missing_episode_stats_cache",
    "discover_items",
    "discover_index_keys",
    "discover_index_meta",
    "discover_series_index",
)
...
def create_table_from_main_schema(conn: sqlite3.Connection, table: str) -> None:
    ...
    target_sql = pattern.sub(f"CREATE TABLE IF NOT EXISTS app_cache.{quote_ident(table)}", source_sql, count=1)
...
def create_indexes_from_main_schema(conn: sqlite3.Connection, table: str) -> None:
    ...
    for name, sql in rows:
        ...
        def replace(match: re.Match) -> str:
            ...
            return f"CREATE {unique}INDEX IF NOT EXISTS app_cache.{quote_ident(name)} ON {quote_ident(table)}"
...
def copy_table(conn: sqlite3.Connection, table: str) -> tuple[int, int]:
    ...
    target_cols = set(table_columns(conn, "app_cache", table))
    ...
    conn.execute(
        f"INSERT OR REPLACE INTO app_cache.{quote_ident(table)} ({col_sql}) "
        f"SELECT {col_sql} FROM main.{quote_ident(table)}"
    )
    target_count = row_count(conn, "app_cache", table)
...
def backup_database(source: Path, backup_dir: Path) -> Path:
    ...
    backup_path = backup_dir / f"{source.stem}.before_app_cache_split.{stamp}.db"
...
def migrate(source: Path, dest: Path, backup_dir: Path, vacuum: bool = True) -> None:
    ...
    try:
        ...
        conn.execute("ATTACH DATABASE ? AS app_cache", (str(dest),))
        moved_tables: list[str] = []
        for table in APP_CACHE_TABLES:
        ...
        if not moved_tables:
            print("no app cache tables found in media library database")
        else:
            for table in moved_tables:
                conn.execute(f"DROP TABLE IF EXISTS main.{quote_ident(table)}")
            conn.commit()
            print(f"dropped from media library db: {', '.join(moved_tables)}")

        conn.execute("DETACH DATABASE app_cache")
        conn.close()
        conn = None
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Move UI/TMDB caches out of media_library_cache.db.")
    parser.add_argument("--source", default=MEDIA_LIBRARY_CACHE_DB_FILE)
    parser.add_argument("--dest", default=APP_CACHE_DB_FILE)
